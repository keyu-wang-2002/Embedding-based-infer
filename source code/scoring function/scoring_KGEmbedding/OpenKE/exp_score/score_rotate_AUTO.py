...
entity_file_name = "./datasetProcess/newData/AUTO/entity2id.txt"
entity_list = []
...
relation_file_name = "./datasetProcess/newData/AUTO/relation2id.txt"
relation_list = []
...
with open('./embed/rotate_AUTO_embed.vec','r',encoding='utf8') as fp:
...
AUTOAxioms_file_name = "./datasetProcess/oriData/AUTOallAxioms.txt"
AUTOAxioms_list = []
with open(AUTOAxioms_file_name, "r") as f:
    for line in f.readlines():
        line = line.strip('\n')
        AUTOAxioms_list.append(line)

axiom2embedd_dict = {}
for axiom in AUTOAxioms_list:
    axiom = axiom.strip("\n")
    ...
    for i in range(3):
        for j in range(200):
            vec.append(axiom_vec[i][j])
    axiom2embedd_dict[axiom] = np.array(vec)
...
# ===================================================================================
Kset = set()
with open("./AUTOMCS/MCS1") as f:
    K1 = f.read().split("\n\n")
    for axiom in K1:
        axiom = axiom.strip("\n")
        Kset.add(axiom)
    f.close()
with open("./AUTOMCS/MCS2") as f:
    K2 = f.read().split("\n\n")
    for axiom in K2:
        axiom = axiom.strip("\n")
        Kset.add(axiom)
    f.close()
with open("./AUTOMCS/MCS3") as f:
    K3 = f.read().split("\n\n")
    for axiom in K3:
        axiom = axiom.strip("\n")
        Kset.add(axiom)
    f.close()
with open("./AUTOMCS/MCS4") as f:
    K4 = f.read().split("\n\n")
    for axiom in K4:
        axiom = axiom.strip("\n")
        Kset.add(axiom)
    f.close()
with open("./AUTOMCS/MCS5") as f:
    K5 = f.read().split("\n\n")
    for axiom in K5:
        axiom = axiom.strip("\n")
        Kset.add(axiom)
    f.close()
with open("./AUTOMCS/MCS6") as f:
    K6 = f.read().split("\n\n")
    for axiom in K6:
        axiom = axiom.strip("\n")
        Kset.add(axiom)
