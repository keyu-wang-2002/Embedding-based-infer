...
transe.save_parameters("./result/transr_transe_UOBM36.json")

# train transr
transr.set_parameters(parameters)
trainer = Trainer(model = model_r, data_loader = train_dataloader, train_times = 1000, alpha = 1.0, use_gpu = True)
trainer.run()
transr.save_checkpoint('./checkpoint/transr_UOBM36.ckpt')

transr.save_parameters('./embed/transr_UOBM36_embed.vec') # 保存嵌入向量

# test the model
transr.load_checkpoint('./checkpoint/transr_UOBM36.ckpt')
